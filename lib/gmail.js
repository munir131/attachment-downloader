...
export function listLabels(auth, gmail) {
...
export async function fetchMailsByMailIds(gmail, auth, spinner, mailList) {
    let results = [];
    let promises = [];
    let processed = 0;
    spinner.text = "Fetching each mail"
    for (const index in mailList) {
      if (mailList[index]) {
...
export function getListOfMailIdByFromId(auth, mailId, maxResults = 500) {
    return new Promise((resolve, reject) => {
